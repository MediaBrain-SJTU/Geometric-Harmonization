...
import numpy as np
from SDCLR.resnet_prune_multibn import prune_resnet18_dual, prune_resnet50_dual, proj_head

class SDCLR(nn.Module):
    def __init__(self, num_class=100, network='resnet18'):
        super(SDCLR, self).__init__()
        if num_class > 100:
            self.backbone = self.get_backbone(network)(pretrained=False, imagenet=True, num_classes=num_class)
        else:
